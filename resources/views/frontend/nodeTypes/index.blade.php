        ...
        <div class="col-md-12">
            @can('node_type_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.node-types.create') }}">
                            ...
                            <tbody>
                                @foreach($nodeTypes as $key => $nodeType)
                                    <tr data-entry-id="{{ $nodeType->id }}">
                                        ...
                                        <td>
                                            @can('node_type_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.node-types.show', $nodeType->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('node_type_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.node-types.edit', $nodeType->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('node_type_delete')
                                                <form action="{{ route('frontend.node-types.destroy', $nodeType->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('node_type_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.node-types.massDestroy') }}",
