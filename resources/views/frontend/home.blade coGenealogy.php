...
class GenogramLayout extends go.LayeredDigraphLayout {
  ...
  function init() {
    var focusId = 1; // default focus on Jesus
      // Filter the nodeDataArray to include only nodes where objectType is "Person"
  var peopleArray = nodeDataArray.filter(function (node) {
    return node.objectType === "Person";
  });
    ...
    familyTreeDiagram =
      new go.Diagram("familyTreeDiagramDiv",
        {
    ...
    familyTreeDiagram.nodeTemplateMap.add("M",  // male
      $(go.Node, "Vertical",
    ...
    familyTreeDiagram.nodeTemplateMap.add("F",  // female
      $(go.Node, "Vertical",
        { locationSpot: go.Spot.Center, locationObjectName: "ICON",
          selectionObjectName: "ICON" },
        new go.Binding("opacity", "hide", h => h ? 0 : 1),
    ...
    familyTreeDiagram.linkTemplateMap.add("Marriage",  // for marriage relationships
      ...
      setupDiagram(familyTreeDiagram, nodeDataArray, focusId);
  }


  // create and initialize the Diagram.model given an array of node data representing people
  function setupDiagram(diagram, array, focusId) {
  // Map 'id' to 'key' for each node data.
  var mappedArray = array.map(function (node) {
    return Object.assign({}, node, { key: node.id });
  });

  // Filter the array to include only nodes where objectType is "Person"
  var peopleArray = mappedArray.filter(function (node) {
    return node.objectType === "Person";
  });
  ...
  diagram.model = new go.GraphLinksModel({
    ...
    copiesArrays: true,
    // Use the filtered array for node data
    nodeDataArray: peopleArray
  });
...
function findMarriage(diagram, a, b) {
  ...
  for (let i = 0; i < nodeDataArray.length; i++) {
    ...
    if (uxs !== undefined) {
      ...
      for (let j = 0; j < uxs.length; j++) {
        const wifeId = uxs[j];
        const wifeData = model.findNodeDataForKey(wifeId);
        if (data.id === wifeId || !wifeData || wifeData.s !== "F") {
          console.log("cannot create Marriage relationship with self or unknown person " + wifeId);
          continue;
        }
        const link = findMarriage(diagram, data.id, wifeId);
        if (link === null) {
          const mlab = { s: "LinkLabel" };
          model.addNodeData(mlab);
          const mdata = { from: data.id, to: wifeId, labelKeys: [mlab.key], category: "Marriage" };
    ...
    if (virs !== undefined) {
      ...
      for (let j = 0; j < virs.length; j++) {
        const husbandId = virs[j];
        const husbandData = model.findNodeDataForKey(husbandId);
        if (data.id === husbandId || !husbandData || husbandData.s !== "M") {
          console.log("cannot create Marriage relationship with self or unknown person " + husbandId);
          continue;
        }
        const link = findMarriage(diagram, data.id, husbandId);
        if (link === null) {
          const mlab = { s: "LinkLabel" };
          model.addNodeData(mlab);
          const mdata = { from: data.id, to: husbandId, labelKeys: [mlab.key], category: "Marriage" };
          model.addLinkData(mdata);
        }
      }
    }
  ...
  for (let i = 0; i < nodeDataArray.length; i++) {
    const data = nodeDataArray[i];
    const motherId = data.m;
    const fatherId = data.f;
    if (motherId !== undefined && fatherId !== undefined) {
      const link = findMarriage(diagram, motherId, fatherId);
      if (link === null) {
        console.log("unknown marriage: " + motherId + " & " + fatherId);
        continue;
      }
      const mdata = link.data;
      if (mdata.labelKeys === undefined || mdata.labelKeys[0] === undefined) continue;
      const mlabkey = mdata.labelKeys[0];
      const cdata = { from: mlabkey, to: data.id };
...
<div id="familyTreeDiagramDiv" style="width: 500px; height: 500px;" ></div>
