...
class UsersController extends Controller
{
    ...
    public function store(StoreUserRequest $request)
    {
        ...
        if ($request->input('user_url', false)) {
            $user->addMedia(storage_path('tmp/uploads/' . basename($request->input('user_url'))))->toMediaCollection('user_url');
        }
    ...
    public function update(UpdateUserRequest $request, User $user)
    {
        ...
        $user->roles()->sync($request->input('roles', []));
        if ($request->input('user_url', false)) {
            if (! $user->user_url || $request->input('user_url') !== $user->user_url->file_name) {
                if ($user->user_url) {
                    $user->user_url->delete();
                }
                $user->addMedia(storage_path('tmp/uploads/' . basename($request->input('user_url'))))->toMediaCollection('user_url');
            }
